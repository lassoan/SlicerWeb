// Command line parsing for the Median Image Filter module, generated by GenerateCLP from
// Modules/CLI/MedianImageFilter/MedianImageFilter.xml of the Slicer this application is built
// from, with the logo included by name rather than by the absolute path the generator wrote. See
// the note in Modules/CLI/CMakeLists.txt on why the generated file is kept here.
#include <cstdio>
#include <cstdlib>
#include <iostream>
#include <string>
...
"  <documentation-url>https://slicer.readthedocs.io/en/latest/user_guide/modules/medianimagefilter.html</documentation-url>\n"
"  <license/>\n"
...
#define GENERATE_DECLARE \
 ...
 \
    ...
    nonbooleanFlags.emplace_back("--neighborhood"); \
    ...
    std::string neighborhoodTemp = "1,1,1"; \
...
#define GENERATE_TCLAP_PARSE \
    ...
    fullDescription += "The MedianImageFilter is commonly used as a robust approach for noise reduction. This filter is particularly efficient against 'salt-and-pepper' noise. In other words, it is robust to the presence of gray-level outliers. MedianImageFilter computes the value of each output pixel as the statistical median of the neighborhood of values around the corresponding input pixel."; \
    if (!std::string("Bill Lorensen (GE)").empty()) \
      { \
      fullDescription += "\nAuthor(s): Bill Lorensen (GE)"; \
      } \
    if (!std::string("This command module was derived from Insight/Examples/Filtering/MedianImageFilter (copyright) Insight Software Consortium").empty()) \
      { \
 ...
 \
    ...
    TCLAP::ValueArg<std::string > neighborhoodArg("", "neighborhood", msg.str(), false, neighborhoodTemp, "std::vector<int>", commandLine); \
 \
    msg.str("");msg << "Input volume to be filtered";    TCLAP::UnlabeledValueArg<std::string> inputVolumeArg("inputVolume", msg.str(), true, inputVolume, "std::string", commandLine); \
 \
    msg.str("");msg << "Output filtered";    TCLAP::UnlabeledValueArg<std::string> outputVolumeArg("outputVolume", msg.str(), true, outputVolume, "std::string", commandLine); \
 \
...
#define GENERATE_TCLAP_ASSIGNMENT \
      ...
      for (unsigned int _j = 0; _j < words.size(); _j++) \
        { \
        neighborhood.emplace_back(atoi(words[_j].c_str())); \
        } \
      } \
