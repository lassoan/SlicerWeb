...
- Only one CLI module is built as C++ (Resample Scalar/Vector/DWI Volume, which Crop Volume needs).
  Each one defines `ModuleEntryPoint` and `XMLModuleDescription`, so a second cannot be linked into
  the same library until those are renamed per module, and each one's command line parser has to be
  generated by GenerateCLP, which needs a host ITK this toolchain does not have - the generated file
  is kept in the tree instead (see `Modules/CLI/`).
