...
async function install(e: ExtensionEntry) {
  ...
  try {
    for (const dep of e.depends ?? []) {
      ...
      if (d && !isInstalled(d)) await install(d);
    }
    const url = wheelUrl(e);
    await runtime.installWheel(url);
    await bridge.evalPython("import slicer; slicer.app.moduleManager().loadModules()");
  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60" @click.self="emit('close')">
    <div class="flex h-[80vh] w-[860px] max-w-[95vw] flex-col rounded-lg border border-input bg-bkg-med shadow-2xl">
      ...
      <div class="flex gap-2 px-4 py-2">
        ...
        <input v-model="indexUrl" class="h-8 w-80 rounded-md border border-input bg-background px-2 text-[12px] text-muted-foreground outline-none" @change="loadIndex" />
      ...
      <div class="min-h-0 flex-1 overflow-y-auto px-4 py-2">
        <div v-for="e in shown" :key="e.name" class="mb-2 flex gap-3 rounded-lg bg-card p-3">
          ...
          <div class="min-w-0 flex-1">
            ...
            <div v-if="e.contributors?.length" class="text-[11px] text-muted-foreground">{{ e.contributors.join(", ") }}</div>
          </div>
