...
// QComboBox / ctkComboBox. items: list of strings or {text, data}.
import { computed } from "vue";

const props = withDefaults(
  defineProps<{ items?: (string | { text: string; data?: unknown })[]; currentIndex?: number; enabled?: boolean; toolTip?: string }>(),
  { items: () => [], currentIndex: 0, enabled: true, toolTip: "" },
);
const emit = defineEmits<{ currentIndexChanged: [number]; currentTextChanged: [string] }>();
const options = computed(() => props.items.map((i) => (typeof i === "string" ? { text: i } : i)));
...
<template>
  <select class="sw-combobox h-7 w-full min-w-0 rounded-md border border-input bg-background px-2 text-[13px] text-foreground outline-none focus:border-primary disabled:opacity-40"
    ...
    <option v-for="(o, i) in options" :key="i" :selected="i === currentIndex">{{ o.text }}</option>
  </select>
</template>
